...
// if  {
    if ((isset($_POST['submit'])) && ($_SERVER["REQUEST_METHOD"] == "POST")) {
        ...
        $errmsg = (($err_name == null && $err_email == null && $err_password == null && $err_phone_number == null && $err_role == null))
           ? (empty($admin_fetch_data_from_db))
                ? ($send_data_to_db->insertindb($admin_data, $column_name, $row_data, $conn) 
                    ? header("location: ./admin_login") 
                    : "Error" . mysqli_error($conn))
                : (($compare_email_status_with_db = !in_array($email, $check_email_exists))
                    ? ($send_data_to_db->insertindb($admin_data, $column_name, $row_data, $conn) && header("location: ./admin_login"))
        ...
        // if  {
            ...
            // print_r($check_email_exists);
            // header("location: ./admin_login.php");
            // print_r($fetch_email_from_id);
            // if(in_array($fetch_email_from_id,$_SESSION['admin'])){
            // $check_email_password_status_with_session_data = !in_array($email, $_SESSION['admin']);
            $check_email_password_status_with_admintable_data = !in_array($email, $check_email_exists);
                // echo $admin_logged_in_email;
                // if{
                //     echo "Ok";
                // }else{
                //     if($check_email_password_status_with_admintable_data){
                //         echo "k";
                //     }else{
                //         echo "Exist";
                //     }
