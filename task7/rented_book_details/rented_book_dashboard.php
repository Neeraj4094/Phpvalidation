...
$fetch_admin_table_data = $fetch_data_from_db->fetchdatafromdb($conn, $tablename);

$rented_book_join = "SELECT rented.*, book.book_name, book.book_category, book.book_author, user.name, user.email, user.phone_no, user.gender,user.status from books_details as book right join rented_book_details as rented on (book.book_id = rented.book_id) left join user_details as user on (rented.user_id = user.user_id) order by rented.rented_id";
$rented_book_join_query = mysqli_query($conn, $rented_book_join);
$right_join_data = mysqli_fetch_all($rented_book_join_query);

foreach ($right_join_data as $data) {
    ...
    <main class="row-span-6 col-span-10  sm:col-span-12 lg:col-span-10 ">
        ...
        <div class="px-2 border-t">
            <div class="px-2 h-80 space-y-3 overflow-y-scroll">
                ...
                <?php
                if ($fetch_admin_table_data == null) {
                    echo $add_books;
                }
                ...
                foreach ($email_list as $email) {
                    ...
                    ?>
                    <?php // }  ?>
                    <div class="<?php echo $searchdata ?> gap-2 py-2 mt-2 px-3 rounded-md border shadow">
                        <div class="flex justify-between items-center p-2 gap-4 w-full">
                            ...
                            <div class="flex items-center gap-10">
                                <div class="flex items-center gap-1">
                                    ...
                                    <button id="<?php echo 'showBookBtn_' . $user_rented_id ?>"
                                        class=" p-1 border rounded-lg bg-slate-50">
                                        <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none"
                                            ...
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M15.996 12h.01M11.995 12h.01M7.995 12h.008"></path>
                                        </svg>
                                    </button>
                                </div>

                                <script>
                                    document.getElementById('<?php echo 'showBookBtn_' . $user_rented_id ?>').addEventListener('click', function () {
                                        var box = document.getElementById('<?php echo 'bookData_' . $user_rented_id ?>');
                                        box.style.display = (box.style.display === 'none' || box.style.display === '') ? 'block' : 'none';
                                    });
                                </script>

                                <div id="<?php echo 'bookData_' . $user_rented_id ?>" class="hidden border p-2">
                                    <div
                                        class="w-full h-full flex flex-col items-center justify-center absolute  left-0 top-0 right-0 bg-black/40">
                                        <div class="relative ">
                                            <form action="rented_book" method="post"
                                                class="font-bold text-2xl z-20 text-slate-400 absolute right-2 top-2">
                                                <button type="submit">
                                            ...
                                            <div class="flex gap-10 items-center border rounded-xl bg-white p-12 relative">
                                                <h2
                                                    class="text-center text-lg underline absolute top-2 left-0 font-bold right-0">
                                                    User Rented books</h2>
                                                <div class="grid text-sm gap-2">
                                                    ...
                                                    foreach ($user_rented_book_details as $user_rented_book_data) {
                                                        ...
                                                        <div class="flex items-center justify-between  gap-2">
                                                            <p
                                                                class="grid w-64 bg-slate-100 border shadow px-3 py-1 rounded-md">
                                                                <span>
                                ...
                                <button id="<?php echo 'showBookBtn_' . $user_lock_data ?>"
                                    class=" border-2 px-4 py-1 rounded-md">
                                    <?php echo $status ?>
                                </button>

                                <script>
                                    document.getElementById('<?php echo 'showBookBtn_' . $user_lock_data ?>').addEventListener('click', function () {
                                        var box = document.getElementById('<?php echo 'bookData_' . $user_lock_data ?>');
                                        box.style.display = (box.style.display === 'none' || box.style.display === '') ? 'block' : 'none';
                                    });
                                </script>

                                <div id="<?php echo 'bookData_' . $user_lock_data ?>" class="hidden">
                                            ...
                                            <div
                                                ...
                                                <div class="grid place-items-center gap-4 pb-2">
                                                    <h2 class="px-6 font-semibold pt-3 text-black text-center">Are you sure u
                                                        want to block this account</h2>
                                                    <form
                                                        action="../user_details/lock_user?rented_id=<?php echo $user_lock_data ?>"
                                                        method="post" class="">
                                                        <button type="submit"
                                    ...
                                    <?php } else { ?>
                                        <div
                                            ...
                                            <div
                                                ...
                                                <div class="grid place-items-center gap-4 pb-2">
                                                    <h2 class="px-6 font-semibold pt-3 text-black text-center">Unblock this
                                                        account</h2>
                                                    <form
                                                        action="../user_details/lock_user?rented_id=<?php echo $user_lock_data ?>"
                                                        method="post" class="">
