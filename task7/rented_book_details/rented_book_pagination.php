...
$total_pages = count($email_list);
foreach ($email_list as $email) {
    $user_emails_list[] = $email;
}
$page_number_data = isset($_GET['user_id']) ? $_GET['user_id'] : 1;
if (empty($page_number_data)) {
    header("location: rented_book_pagination");
}

foreach ($email_list as $email) {
    $user_id_data = $fetch_data_from_db->fetch_data('user_details', 'user_id', $email, $conn, 'email');
    $user_id = isset($user_id_data[0][0]) ? $user_id_data[0][0] : '';
    $user_id_list[] = $user_id;

}
if (!in_array($page_number_data, $user_id_list)) {
    print_r($user_id_list);
    header("location: /phpprogramms/task7/rented_book_details/rented_book_pagination");
}
?>
...
<body class="grid grid-cols-12 grid-rows-6 w-full h-full">
        ...
        <div class="px-2 border-t">
                                    ...
                                    <?php if ($user_status == 'Active') { ?>
                                        <div
                                            ...
                                            <div
                                                ...
                                                <form action="rented_book_pagination" method="post"
                                                ...
                                                <div class="grid place-items-center gap-4 pb-2">
                                                    <h2 class="px-6 font-semibold pt-3 text-black text-center">Are you sure u want
                                                        to block this account</h2>
                                                    <form action="../user_details/lock_user?rented_id=<?php echo $user_lock_data ?>"
                                                        method="post" class="">
                                                        <button type="submit"
                                    ...
                                    <?php } else { ?>
                                        <div
                                            ...
                                            <div
                                                ...
                                                <div class="grid place-items-center gap-4 pb-2">
                                                    <h2 class="px-6 font-semibold pt-3 text-black text-center">Unblock this account
                                                    </h2>
                                                    <form action="../user_details/lock_user?rented_id=<?php echo $user_lock_data ?>"
                                                        method="post" class="">
                                                        <button type="submit"
            ...
            <div class="flex gap-1 fixed bottom-3 left-1/2 right-1/2">
                ...
                for ($i = 0; $i < $total_pages; $i++) {
                    ...
                    echo '<a href="?user_id=' . $user_id . '" class="border p-1 px-2 rounded-sm">' . ($i + 1) . '</a>';
                }
                ?>
            </div>
        </div>
    ...
    <!-- <footer class=" fixed flex items-center justify-center text-center bottom-0 border bg-slate-200 w-full">
        <p class="ml-28 text-xs">Ucodesoft Software Pvt Lmd Copyright&copy; 2024 </p>
    </footer> -->
</body>
