...
$err_category = $err_book = $err_author = $err_book_copies = $err_image = $err_book_price = $err_book_desciption = $errmsg = "";
$total_book_copies = 0;
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && (isset($_POST["add_books"]))) {
    ...
    if ($err_category == null && $err_book == null && $err_author == null && $err_book_copies == null && $err_book_price == null && $err_book_desciption == null) {
        ...
        if (empty($book_name_array)) {
            $book_column = ['book_name', 'book_author', 'book_category', 'book_copies', 'book_price', 'description', 'book_image_name', 'created_date', 'modified_Date', 'book_unique_image_name'];
            ...
            $errmsg = !empty($id) //If id is not empty then the data will be "updated" otherwise it will go to the next condition which is used for "inserting" the data 
                ? (
                    ...
                    ? (
                        ($update_book_column = ['book_name', 'book_author', 'book_category', 'book_copies', 'book_price', 'description', 'book_image_name', 'modified_Date', 'book_unique_image_name']) &&
