...
$leftjoin = "select login.*, user_image,  login.created_date ,image.Modified_Date from registeration_login as login left join record_of_image as image on (login.id= image.user_id) order by login.id";
$leftjoinquery = mysqli_query($con, $leftjoin);

$imagedata = $dbdata->fetchdatafromdb($con, 'record_of_image');
...
foreach ($totaldata as $item) {
    if (in_array($_SESSION['email'], $item)) {
        $data1 = $item;
        $dataid = isset($data1['ID'])?$data1['ID']:'';
            ...
            <div class="flex justify-between items-center border-b-2 py-3 px-2 ">
                ...
                <div class="w-10 h-10 rounded-full border">
                    ...
                    if ($dataimage != "") {
                        ...
                        $res = "<img src='$images' alt='Image ' class='w-10 h-10 object-cover rounded-full flex items-center justify-center'>";
                        echo "$res<br>";
                    } else {
            ...
            <div class="px-2 border-t">
                ...
                $no_delete = '<svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"><path d="M3.93931 5L2.21966 3.28032C1.92677 2.98743 1.92678 2.51255 2.21968 2.21966C2.51257 1.92677 2.98745 1.92678 3.28034 2.21968L21.7801 20.7198C22.073 21.0127 22.073 21.4876 21.7801 21.7805C21.4872 22.0734 21.0123 22.0734 20.7194 21.7805L18.5293 19.5903C17.9867 21.0098 16.6131 22 15.0263 22H8.97369C7.04254 22 5.42715 20.5334 5.24113 18.6112L4.06908 6.5H2.75C2.33579 6.5 2 6.16421 2 5.75C2 5.33579 2.33579 5 2.75 5H3.93931ZM17.2782 18.3392L15 16.0609V17.25C15 17.6642 14.6642 18 14.25 18C13.8358 18 13.5 17.6642 13.5 17.25V14.5609L10.5 11.5608V17.25C10.5 17.6642 10.1642 18 9.75 18C9.33579 18 9 17.6642 9 17.25V10.0608L5.59074 6.65147L6.73416 18.4667C6.84577 19.62 7.815 20.5 8.97369 20.5H15.0263C16.185 20.5 17.1542 19.62 17.2658 18.4667L17.2782 18.3392ZM13.5 10.3185L15 11.8186V9.75C15 9.33579 14.6642 9 14.25 9C13.8358 9 13.5 9.33579 13.5 9.75V10.3185ZM18.4239 6.5L17.6525 14.4711L19.0265 15.8452L19.9309 6.5H21.25C21.6642 6.5 22 6.16421 22 5.75C22 5.33579 21.6642 5 21.25 5H15.5C15.5 3.067 13.933 1.5 12 1.5C10.067 1.5 8.5 3.067 8.5 5H8.18156L9.68153 6.5H18.4239ZM14 5H10C10 3.89543 10.8954 3 12 3C13.1046 3 14 3.89543 14 5Z" fill="currentColor"></path></svg>';
                    foreach ($totaldata as $item) {
                        ...
                        $registerid = $item['ID'];
                        $select = "SELECT created_date from registeration_login where ID = $registerid";
                        $query = mysqli_query($con,$select);
                        $fetchid = mysqli_fetch_assoc($query);

                        $cr['create'] = $item['created_date'];
                        $md['modify'] = $item['Modified_Date'];
                        $createddate = $date->date_time_in_india($cr);
                        $modifieddate = $date->date_time_in_india($md);
                        $item['createddate_time'][$registerid] = $createddate;
                        $item['modifieddate_time'][$registerid] = $modifieddate;
                        if($item['modifieddate_time'][$registerid] == null){
                            $item['modifieddate_time'][$registerid] = $item['createddate_time'][$registerid];
                        }
                        
                        
                        $check = in_array($_SESSION["email"], $item);
                        $checkid = $check?$dataid:$itemid;
                        ...
                        <div class="grid gap-2 py-2 sm:block md:grid mt-2 <?php echo $boxColor  ?>  rounded-md border w-full shadow">
                            <div class=" flex justify-between items-center p-2 gap-4 w-full ">
                                <div class="w-full flex justify-between items-center gap-4">
                                    ...
                                    <div class=" w-full">
                                        <div class="flex gap-20">
                                            ...
                                            <div class="space-y-1">
                                                <p>Created :- <?php echo $item['createddate_time'][$registerid] ?></p>
                                                <p>Modified :- <?php echo $item['modifieddate_time'][$registerid]; ?></p>
                                            </div>
                                        </div>
                                ...
                                <div class="flex items-center justify-between gap-6">
                                    <p class=" bg-indigo-500 text-white px-3 py-1 rounded-full"> <?php echo $useroccupation ?></p>
                                    <p class=" bg-purple-500 text-white px-3 py-1 rounded-full">
                                        <?php echo $userrole ?></p>

                                    <form action="update.php?id=<?php  echo $checkid ?>" method="post">
                                    ...
                                    <form action="delete.php?id=<?php echo $checkid ?>" method="post">
                                        <button data-toggle="tooltip" data-placement="top" title="Delete" class="border-2 px-4 py-1 rounded-md"><?php echo $userdelete ?></button>
                                    </form>
                                </div>
                            </div>
                        </div>
